function y = fixedTrn_ncc_sa_2std(data)
        ...
        for i = 1:2 * 7
            for j = 1:epochsNumber
                ...
                if isnan(epoch(1,1))
                    ...
                    epochs_psd(i*epochsNumber + j, 1) = mean(epoch);
                    epochs_psd(i*epochsNumber + j, 2) = skewness(epoch);
                    epochs_psd(i*epochsNumber + j, 3) = kurtosis(epoch);
                    epochs_psd(i*epochsNumber + j, 4) = std(epoch);
                    epochs_psd(i*epochsNumber + j, 5) = min(epoch);
                    epochs_psd(i*epochsNumber + j, 6) = max(epoch);
                    epochs_psd(i*epochsNumber + j, 7) = max(epoch) - min(epoch);                   
                end                            
            end        
        end
